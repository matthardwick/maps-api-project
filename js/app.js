...
function viewModel() {
    ...
    function loadFourSquare() {
        ...
        var urlStart = 'https://api.foursquare.com/v2/venues/explore?client_id=';
        // api key and secret
        var clientId = 'LY5SLL43AUDYITSBN5YE5KY2DN5V5TK5KA2HFEX310H33ISP';
        var clientSecret = '&client_secret=NTCWDJRCOT45OTIKAEKBDVKDZQTDN0DC13SR3ZD23HWJAVDS';
        ...
        var limit = '&limit=10&'
        // compile
        var fourSquareUrl = urlStart + clientId + clientSecret + version + location + limit + search;
    ...
    function getMarkerInfo(data) {
        ...
        var infoWindowData = 
            ...
            '<a href ="' + data.fourSquareUrl + '">' + data.venueName + '</a>' + '<div class="list-rate-position">' +
            '<span class="venue-rating badge">' + data.rating + 
            '</span>' + '</div>' + '</div>' + 
            '<div class="venue-category">' + data.category + '</div>' +
            '<div class="venue-address">' + data.formattedAddress + '</div>' + '<div class="venue-phone">' + 	
            data.formattedPhone	+ '</div>' + '<div class="venue-url">' + data.url + '</div>' + '</div>';
    ...
    that.venueFilter = function () {
        ...
        for (var i in that.dataList()) {
            if (that.dataList()[i].venueName.toLowerCase().indexOf(filterResults) != -1 ||
                that.dataList()[i].category.toLowerCase().indexOf(filterResults) != -1) {
